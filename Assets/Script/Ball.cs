...
public class Ball : MonoBehaviour
{
    ...
    public bool bonusGoal; //untuk pertemuan 8
    ...
    void OnCollisionEnter2D(Collision2D col) 
    {
        ...
        if (col.gameObject.tag == "Racked Red" && !isBounce)
        {
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        if (col.gameObject.tag == "Goal 1")
        {
            ...
            GameManager.instance.SpawnBall();
            Destroy(gameObject);
            if (GameManager.instance.goldenGoal) //Fungsi ini di gunakan untuk memvalidasi kondisi sedang dalam mode golden goal
            {
    ...
    {
        isBounce = true;
        yield return new WaitForSeconds(1f); //Kalau menggunakan ienumerator, sebagai returnnya harus menggunakan yeild
        isBounce = false;
    }
}
